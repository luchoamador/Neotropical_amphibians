...
mean(a) #0.0269844
max(a) #0.202941
range(a) #0.000000 0.202941
b <- dnorm(a)

ggplot(matrix, aes(x = a)) +
...
##pi is not normally distributed

############Generalized Linear Models####################################
###Amphibian body size
# Fit regression line
size <- glm(pi~Body_size_mm, data = matrix)
summ(size)
summary(size)
...
eq
# Plot
pi_size + geom_abline(intercept = 2.793722e-02, slope = -1.953164e-05, linetype="dashed", linewidth=0.5)+
  ggtitle(eq)

###Anura
anura <- read.csv(file = "Anura.csv", sep=";")
an1 <- glm(pi~Body_size_mm, data = anura)
summary(an1)
summ(an1)
...
ca1 <- glm(pi~Body_size_mm, data = caudata)
summary(ca1)
summ(ca1)
...
ADm <- glm(pi~Development_Mode, data = matrix)
summary(ADm)
summ(ADm)
...
#anura
aDm <- glm(pi~Development_Mode, data = anura)
summ(aDm)
summary(aDm)
...
##############################################################
#######Amphibia habitat#######################################
am_h <- glm(pi ~ Microhabitat, data=matrix)
summary(am_h)
summ(am_h)
...
##############################################################
#######Amphibia activity#######################################
am_a <- glm(pi ~ Activity, data=matrix)
summ(am_a)
summary(am_a)
...
am_seqs <- glm(pi ~ Seqs, data=matrix)
summ(am_seqs)
summary(am_seqs)
...
#Anura
an_seqs <- glm(pi ~ Seqs, data=anura)
summ(an_seqs)
summary(an_seqs)
...
#Caudata
c_seqs <- glm(pi ~ Seqs, data=caudata)
summ(c_seqs)
summary(c_seqs)
...
am_el <- glm(pi ~ Elevation_mean, data=matrix)
summ(am_el)
summary(am_el)
...
#anura
an_el <- glm(pi ~ Elevation_mean, data=anura)
summ(an_el)
summary(an_el)
...
#caudata
ca_el <- glm(pi ~ Elevation_mean, data=caudata)
summ(ca_el)
summary(ca_el)
...
am_minel <- glm(pi ~ Elevation_min, data=matrix)
summ(am_minel)
summary(am_minel)
...
#anura
an_minel <- glm(pi ~ Elevation_min, data=anura)
summ(an_minel)
summary(an_minel)
...
#caudata
ca_minel <- glm(pi ~ Elevation_min, data=caudata)
summ(ca_minel)
summary(ca_minel)
...
am_maxel <- glm(pi ~ Elevation_max, data=matrix)
summ(am_maxel)
summary(am_maxel)
...
an_mx_el <- glm(pi ~ Elevation_max, data=anura)
summ(an_mx_el)
summary(an_mx_el)
...
ca_mx_el <- glm(pi ~ Elevation_max, data=caudata)
summ(ca_mx_el)
summary(ca_mx_el)
...
am_lat <- glm(pi ~ Lat_mean, data=matrix)
summ(am_lat)
summary(am_lat)
...
#Anura
an_lat <- glm(pi ~ Lat_mean, data=anura)
summ(an_lat)
summary(an_lat)
...
#Caudata
ca_lat <- glm(pi ~ Lat_mean, data = caudata)
summ(ca_lat)
summary(ca_lat)
...
am_ilat <- glm(pi ~ Lat_realmin, data=matrix)
summ(am_ilat)
summary(am_ilat)
...
an_ilat <- glm(pi ~ Lat_realmin, data=anura)
summ(an_ilat)
summary(an_ilat)
...
ca_ilat <- glm(pi ~ Lat_realmin, data=caudata)
summ(ca_ilat)
summary(ca_ilat)
...
am_xlat <- glm(pi ~ Lat_realmax, data=matrix)
summ(am_xlat)
summary(am_xlat)
...
an_xlat <- glm(pi ~ Lat_realmax, data=anura)
summ(an_xlat)
summary(an_xlat)
...
ca_xlat <- glm(pi ~ Lat_realmax, data=caudata)
summ(ca_xlat)
summary(ca_xlat)
...
am_sr <- glm(pi ~ log(Area_km2), data=matrix)
summ(am_sr)
summary(am_sr)
...
#Anura
an_sr <- glm(pi ~ log(Area_km2), data=anura)
summ(an_sr)
summary(an_sr)
...
#Caudata
ca_sr <- glm(pi ~ log(Area_km2), data=caudata)
summ(ca_sr)
summary(ca_sr)
...
#Anuran families
an.fam <- glm(pi ~ Family, data=anura)
summ(an.fam)
summary(an.fam)

####Conservation status
uicn <- glm(pi ~ IUCN, data=matrix)
summ(uicn)
summary(uicn)
...
uicn_c <- glm(pi ~ IUCN, data=caudata)
summ(uicn_c)
summary(uicn_c)
...
#####Biogeographic_Unit#####
bu1 <- glm(pi ~ Biogeographic_Unit, data = matrix)
summ(bu1)
summary(bu1)
...
bu2 <- glm(pi ~ Biogeographic_Unit, data = anura)
summ(bu2)
summary(bu2)
...
bu3 <- glm(pi ~ Biogeographic_Unit, data = caudata)
summ(bu3)
summary(bu3)
...
amph.neot.physig <- phylosig(neot.tree, Nucleotide.diversity, nsim = 999, method = 'lambda', test = TRUE)
amph.neot.physig
amph.neot.physig$logL

plot.phylosig(amph.neot.physig)
